import {reinstall as reinstallWICP} from "./scripts/canisters/dicp";
import {reinstall as reinstallMonitor} from "./scripts/canisters/monitor"
...
export const reinstall_all = async (options?: CanisterReinstallOptions) => {
    ...
    const get_jobs = function* () {
        ...
        function* step1() {
            // dft dicp
            if (options && options.canisters?.dicp?.reinstall) {
                yield reinstallWICP({
                    ...options,
                }, options.canisters.dicp.initOptions);
            }
            // monitor
            if (options && options.canisters?.monitor) {
                yield reinstallMonitor({
                    ...options,
                });
            }
        }
