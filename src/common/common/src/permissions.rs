...
use crate::named_principals::{
    get_named_principals, is_named_principal, PRINCIPAL_NAME_ADMIN, PRINCIPAL_NAME_MONITOR,
};
...
pub fn must_be_monitor(caller: &Principal) -> ServiceResult<()> {
    must_not_anonymous(caller)?;
    if !is_monitor(caller) {
        return Err(CommonError::Unauthorized);
    }
    Ok(())
}
...
pub fn is_monitor(user: &Principal) -> bool {
    is_named_principal(PRINCIPAL_NAME_MONITOR, user)
}
